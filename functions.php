...
{
	wp_enqueue_script('vue-script', get_theme_file_uri('assets/js/app.js'), [], true, true);
	...
	wp_enqueue_style('app-css', get_theme_file_uri('assets/css/app.css'));
}
...
/**
 * Register donors custom post type
 */
...
/**
 * Register restful API endpoint for all donation actions.
 */
add_action('rest_api_init', function () {

	$validation = [
		'required' => function ($value, $request, $param) {
			if ($value == '' || $value == 'null') {
				return new WP_Error(
					'rest_invalid_param',
					sprintf(__('%s is required.'), $param),
					['status' => 422]
				);
			}
		},
		'gt:0' => function ($value, $request, $param) {
			if (!is_numeric($value) || (is_numeric($value) && (int) $value <= 0)) {
				return new WP_Error(
					'rest_invalid_param',
	...
	register_rest_route('donation', '/submit', [
		'methods' => WP_REST_Server::CREATABLE,
		'callback' => function (WP_REST_Request $request) {

			$donorId = wp_insert_post([
				'post_title' => $request->get_param('first_name') . ' ' . $request->get_param('last_name'),
				'post_type' => 'donors',
				'post_status' => 'publish',
			], true);

			add_post_meta($donorId, 'first_name', $request->get_param('first_name'));
			add_post_meta($donorId, 'last_name', $request->get_param('last_name'));
			add_post_meta($donorId, 'email', $request->get_param('email'));
			add_post_meta($donorId, 'phone', $request->get_param('phone'));
			add_post_meta($donorId, 'donated', $request->get_param('donated'));
			add_post_meta($donorId, 'payment_method', $request->get_param('payment_method'));

			$donor = get_post($donorId);
		...
		'args' => [
			'first_name' => [
				...
				'validate_callback' => $validation['required'],
			],
			'last_name' => [
				'required'      => true,
				'type'          => 'string',
			...
			'donated' => [
				...
				'type'          => 'int',
				'description'   => 'phone',
				'validate_callback' => $validation['gt:0'],
			],
			'payment_method' => [
				'required'      => true,
				'type'          => 'string',
...
/**
 * Add short code donation box
 */
add_shortcode('donation-box', function () {

	$donationPostMetas = array_filter(get_post_meta(get_the_ID()), function ($key) {
		return preg_match('/donation/i', $key);
	}, ARRAY_FILTER_USE_KEY);

	$donationProps = '';

	foreach ($donationPostMetas as $key => $value) {
		$prop = end(explode(':', $key));

		$donationProps .= " {$prop}=\"{$value[0]}\"";
	}

	return "<donation-box {$donationProps}/>";
});
